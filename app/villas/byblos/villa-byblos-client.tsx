...
import type { PublicVillaMedia } from "@/lib/public-villa-media";
...
export default function VillaByblosClient({ initialMedia }: { initialMedia: PublicVillaMedia[] }) {
  ...
  const heroImg      = initialMedia[0]?.file_url ?? "";
  const galleryMedia = initialMedia.slice(1);
  const fromPrice = formatVillaFromPrice("Villa Byblos", pricing);
  const instantHeroBadge = instantBookingEnabledForVilla("Villa Byblos", instantBookingFlags);
      ...
      <section className="oraya-section-tone" style={{ paddingTop: "80px", backgroundColor: MIDNIGHT, minHeight: "65vh", display: "flex", flexDirection: "column" }}>
        <div style={{
          ...
          {heroImg && (
            <Image
              src={heroImg}
              alt="Villa Byblos garden estate"
              fill
              priority
              quality={82}
              sizes="100vw"
      ...
      <section className="oraya-section-tone" style={{ backgroundColor: WHITE, padding: "3rem" }}>
        <div style={{ maxWidth: "760px", margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "1rem 2.5rem" }}>
            <a href="/house-book/byblos" className="no-underline uppercase oraya-link-cta" style={{ fontFamily: LATO, fontSize: "11px", letterSpacing: "2px", color: GOLD, paddingBottom: "2px" }}>
              The House Book
            </a>
            <a href="/explore/byblos" className="no-underline uppercase oraya-link-cta" style={{ fontFamily: LATO, fontSize: "11px", letterSpacing: "2px", color: GOLD, paddingBottom: "2px" }}>
              Explore Byblos
            </a>
          </div>
        </div>
