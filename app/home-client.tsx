...
export default function HomeClient({ initialMedia }: HomeClientProps) {
  ...
  const [loadedVillaImages, setLoadedVillaImages] = useState<Record<string, boolean>>({});
  ...
  return (
    <div style={{ overflowX: "hidden" }}>
      ...
      >
        {initialMedia.hero && (
          <Image
            src={initialMedia.hero}
            alt="Oraya boutique villa at night"
            fill
            priority
            quality={82}
            sizes="100vw"
      ...
      <section id="villas" className={`oraya-section-tone ${SEC.px} ${SEC.medY}`} style={{ backgroundColor: WHITE }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 max-w-[1100px] mx-auto items-stretch min-w-0" style={{ gap: "2px" }}>
          {villaMeta.map(({ key, tag, name, loc, feats, href, gradient, label }) => {
            const img = key === "mechmech" ? initialMedia.mechmech : initialMedia.byblos;
              ...
              >
                {img && (
                  <Image
                    ...
                    onLoad={() => setLoadedVillaImages((current) => ({ ...current, [key]: true }))}
                    style={{
                      objectFit: "cover",
                      objectPosition: "center",
                      opacity: loadedVillaImages[key] ? 1 : 0,
                      transition: "opacity 350ms ease",
                    }}
                  />
                )}
