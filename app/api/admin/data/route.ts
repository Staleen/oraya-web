...
export async function GET() {
  ...
  const { data: bookings, error: bookingsError } = await supabaseAdmin
    .from("bookings")
    .select(
      "id, member_id, villa, check_in, check_out, sleeping_guests, day_visitors, event_type, message, addons, addons_snapshot, pricing_subtotal, pricing_snapshot, status, created_at, guest_name, guest_email, guest_phone, guest_country"
    )
