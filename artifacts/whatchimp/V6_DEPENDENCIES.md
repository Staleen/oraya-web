...
**Status: HALTED — NOT OPERATOR-REPAIRABLE AS SHIPPED; not approved for human testing (2026-07-03, round trip #2).** Round trip #1 proved WhatChimp's **import** keeps only the first serialized connection per input socket ([`roundtrips/ROUNDTRIP_1_FINDINGS.md`](roundtrips/ROUNDTRIP_1_FINDINGS.md); pre-hybrid candidate pinned at `roundtrips/Oraya_natural_intake_v6.roundtrip-1.import-candidate.txt`, SHA-256 `72578281…08A4`), so the artifact was rebuilt to the operator-selected Option A hybrid (branch-local tail clones + 11 declared hubs + an 18-edge operator redraw checklist; current bytes: **165 nodes, 182 output connections**, SHA-256 `0066192D4487ED1AC8A95B00E22DEF8B754B1FB458E0025B178E8B125C913A39`). **Round trip #2 then proved the redraw plan is not executable: the WhatChimp editor REFUSES to draw Condition → Condition connections** (*"This will make an infinite loop. Place a button/list/section/interactive between these two nodes."*) — and **11 of the 18 planned draws are Condition → Condition** (items #1–4, #6–8, #12, #14–16), with the remaining 7 type pairs unproven. The v5.5 `#440` 5-parent merge previously cited as drawability evidence is itself Condition → Condition ×5: a merge surviving in a saved export does **not** prove the present editor can draw it. Full audit, quantified behavior-preserving rebuild direction (Condition-clone cascade: +21 nodes, 39 draws, all onto interactive/Text nodes), and the mandatory editor drawability probe plan: [`roundtrips/ROUNDTRIP_2_FINDINGS.md`](roundtrips/ROUNDTRIP_2_FINDINGS.md). The validator's new `redraw-drawability` check fails this candidate **by design** (11 errors + 7 unproven-pair warnings); the profile carries the evidence in `importGraphContract.editorRejectedDrawPairs` / `editorProvenDrawPairs`. The un-repaired import remains safe by construction (happy-path-first hub edges, opening-question `/book` safety link, no confirmation claims), but **do not import, draw, or test this candidate further; do not insert interactive nodes to bypass the editor warning; production stays untouched. Next step is the probe gate + a human architecture decision (KNOWN_BUGS #10, DECISIONS_LOG 2026-07-03).**
