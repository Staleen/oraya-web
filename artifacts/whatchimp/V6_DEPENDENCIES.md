...
**Status: NOT IMPORT-SAFE — do not import toward production (2026-07-03, authenticated round trip #1).** The operator's import → save → close → reopen → re-export test succeeded mechanically but WhatChimp kept only the first serialized connection per input socket and silently removed the artifact's 32 convergence edges (all 16 multi-parent nodes reduced to one parent; 16 unintended terminals; validator on the re-export: 96 errors; simulator: 6/42). Evidence and the complete lost-edge table: [`roundtrips/ROUNDTRIP_1_FINDINGS.md`](roundtrips/ROUNDTRIP_1_FINDINGS.md). The validator's `single-parent-contract` check now fails this candidate by design (exactly 16 violations). The structural rebuild direction (bounded operator redraw of hub merges vs approved behavior reduction to a pure tree — a full clone-unroll is 492,864 nodes, infeasible) awaits the operator decision recorded in DECISIONS_LOG 2026-07-03. Round-trip verification of any future candidate is machine-checked with `node scripts/compare-whatchimp-roundtrip.mjs <candidate> <re-export>`.
