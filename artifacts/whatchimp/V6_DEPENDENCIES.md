...
**Status: HYBRID IMPORT + REDRAW CANDIDATE — round trip #2 pending (2026-07-03, Option A rebuild).** Authenticated round trip #1 proved WhatChimp's **import** keeps only the first serialized connection per input socket (evidence and complete lost-edge table: [`roundtrips/ROUNDTRIP_1_FINDINGS.md`](roundtrips/ROUNDTRIP_1_FINDINGS.md); the pre-hybrid candidate is pinned byte-exact at `roundtrips/Oraya_natural_intake_v6.roundtrip-1.import-candidate.txt`, SHA-256 `72578281…08A4`), while editor-drawn merges are export-proven to survive (v5.5 `#440` with 5 parents; v4.3.3 multi-input). The artifact is therefore rebuilt to the operator-selected **Option A hybrid architecture** (DECISIONS_LOG 2026-07-03): every small linear tail (escalation, Edit large-group, Edit handoff) is cloned branch-locally as single-parent, and the **11 irreducible hub merges** are declared in the profile's `importGraphContract.approvedHubMerges` — after import, the operator draws **exactly 18 connections** in the editor following the machine-generated [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md). Current canonical artifact: **165 nodes, 182 output connections**, SHA-256 `0066192D4487ED1AC8A95B00E22DEF8B754B1FB458E0025B178E8B125C913A39` (printed by the generator, which asserts the redraw plan equals the set of all beyond-first hub edges). The un-repaired import is safe by construction: the first-listed edge on every hub is the happy path (the full main scenario runs without any redraw), the opening question carries the `https://stayoraya.com/book` safety link, and no terminal claims a confirmed booking. Validator on the artifact: exit 0, 0 errors, 0 warnings under `--strict-binding`; the `single-parent-contract` check accepts only the declared hubs at their exact inbound counts, so an un-repaired re-export (hub-count drift) is machine-detected. **Import-safety may not be claimed until round trip #2 — import → 18 redraws → save → close → reopen → export → `node scripts/compare-whatchimp-roundtrip.mjs` PRESERVED exit 0 + validator `--strict-binding` exit 0 — passes on a fresh disposable bot (V6_ROUNDTRIP_CHECKLIST.md).**
