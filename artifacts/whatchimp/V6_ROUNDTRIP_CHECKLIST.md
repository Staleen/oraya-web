...
> **Current candidate (2026-07-03): CORRECTED-RULE HYBRID — import + 34 proven-operation redraws.** Round trip #1 proved the import keeps only the first serialized connection per input socket ([`roundtrips/ROUNDTRIP_1_FINDINGS.md`](roundtrips/ROUNDTRIP_1_FINDINGS.md)). Round trip #2 halted at redraw item #1 and the operator's same-day probe matrix corrected the platform rule: the editor rejects only a **second Condition-source parent** on a Condition — six connection operations are live-proven ([`roundtrips/ROUNDTRIP_2_FINDINGS.md`](roundtrips/ROUNDTRIP_2_FINDINGS.md) Addendum). The rebuilt `Oraya_natural_intake_v6.txt` (181 nodes, 214 connections, 14 terminals, SHA-256 `AB456A89…AB0C`) clones every Condition that would need an extra Condition-source parent (16 guest-invisible clones, ids 750–765) so that **after every import the operator draws exactly 34 connections — every one a live-proven operation, never Condition → Condition —** following [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md), before sections B/C/D apply. The un-repaired import is safe by construction (happy path complete, opening-question safety link, no fake confirmations); the validator detects an un-repaired re-export as hub-count drift.

## Round trip #3 — acceptance procedure for the corrected-rule candidate (mandatory before any section C testing)

1. Use a **fresh disposable test bot** (not the round-trip-#1 or #2 bots, not production).
2. Import `Oraya_natural_intake_v6.txt` → **draw the 34 connections** exactly as listed in [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md) (only that list — no other edits) → **Save** → close the editor completely → reopen → **export**.
...
3. Confirm the import persisted structurally. At this point the API nodes still display the **Production** integration names carried in the export — that is expected before rebinding:
   ...
   - The multi-input merge points arrive from import with **only their first inbound arrow** — that is the expected hybrid state, not a failure. **Draw the missing connections exactly per the regenerated [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md), then save → close → reopen and confirm every drawn arrow survived.** If the editor refuses any listed connection (e.g. the infinite-loop warning), or a drawn arrow disappears after save, **stop and report immediately** — do not insert nodes to work around it (round trip #2 halted on exactly such a refusal; see `roundtrips/ROUNDTRIP_2_FINDINGS.md`).
