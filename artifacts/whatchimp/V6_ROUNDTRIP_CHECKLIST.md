...
11. **No-dead-end / HTTP-failure behavior (critical platform unknown)** — temporarily point a spare cloned TEST Stay Intent integration at an invalid path (e.g. `/api/butler/does-not-exist`) on a throwaway copy of the flow, send a stay message, and observe whether WhatChimp **continues to the next node** when the HTTP call fails. The repo simulator assumes it continues (the graph has no status branching); if the live platform instead halts the conversation on HTTP error, stop and report — that changes the failure posture of every API node. Restore the correct endpoint afterwards.
