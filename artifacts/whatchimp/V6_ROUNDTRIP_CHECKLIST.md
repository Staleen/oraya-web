...
> **⛔ ROUND TRIP #2 HALTED (2026-07-03): the current candidate and its redraw checklist are NOT approved for human testing.** The operator attempted redraw item #1 (Condition FALSE output → Condition input) on a fresh disposable bot and the WhatChimp editor **refused the connection**: *"This will make an infinite loop. Place a button/list/section/interactive between these two nodes."* A machine audit of all 18 planned draws shows **11 are Condition → Condition** (items #1–4, #6–8, #12, #14–16) and none of the remaining 7 type pairs has proven drawability — the v5.5 5-parent merge previously cited as evidence is itself Condition → Condition ×5, and export survival does not prove present-editor drawability. Evidence + full audit + quantified alternatives + the drawability probe plan: [`roundtrips/ROUNDTRIP_2_FINDINGS.md`](roundtrips/ROUNDTRIP_2_FINDINGS.md). The validator now fails this candidate **by design** (11 `redraw-drawability` errors). **Do not draw, test, or import this candidate further; do not insert interactive nodes to bypass the warning.** Sections below apply only to a FUTURE candidate whose redraw plan uses exclusively operator-proven drawable connections.

## Round trip #2 — acceptance procedure (for the NEXT candidate, after the probe gate)

0. **Probe gate first (blocking):** run the editor drawability probes in [`roundtrips/ROUNDTRIP_2_FINDINGS.md`](roundtrips/ROUNDTRIP_2_FINDINGS.md) §6 and record the results in the profile (`editorProvenDrawPairs` / `editorRejectedDrawPairs`); the generator is rebuilt only against proven pairs, then the artifact + checklist are regenerated.
1. Use a **fresh disposable test bot** (not a previously-used bot, not production).
2. Import the regenerated candidate → **draw the connections** exactly as listed in the regenerated [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md) (only that list — no other edits) → **Save** → close the editor completely → reopen → **export**.
