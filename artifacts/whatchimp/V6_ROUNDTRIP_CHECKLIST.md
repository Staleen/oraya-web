...
> **Current candidate (2026-07-03): FULL CONDITION-CLONE CASCADE — import + 39 proven-operation redraws.** Round trip #1 proved the import keeps only the first serialized connection per input socket ([`roundtrips/ROUNDTRIP_1_FINDINGS.md`](roundtrips/ROUNDTRIP_1_FINDINGS.md)). Round trip #2 halted at redraw item #1 ([`roundtrips/ROUNDTRIP_2_FINDINGS.md`](roundtrips/ROUNDTRIP_2_FINDINGS.md)). Round trip #3 halted again — 3 of the 34 draws targeted Condition inputs that already carried their import-kept parent — and corrected the platform rule: **each Condition accepts at most ONE inbound connection TOTAL, any source type** ([`roundtrips/ROUNDTRIP_3_FINDINGS.md`](roundtrips/ROUNDTRIP_3_FINDINGS.md)). The rebuilt `Oraya_natural_intake_v6.txt` (186 nodes, 224 connections, 14 terminals, SHA-256 `1C7335ED…EDD48`) clones every Condition with more than one inbound edge (21 guest-invisible clones, ids 750–770) so that **after every import the operator draws exactly 39 connections — every one a proven operation, never into a Condition —** following [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md), before sections B/C/D apply. The un-repaired import is safe by construction (happy path complete, opening-question safety link, no fake confirmations); the validator detects an un-repaired re-export as hub-count drift.

## Round trip #4 — acceptance procedure for the full-cascade candidate (mandatory before any section C testing)

1. Use a **fresh disposable test bot** (not the round-trip-#1/#2/#3 bots, not production).
2. Import `Oraya_natural_intake_v6.txt` → **draw the 39 connections** exactly as listed in [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md) (only that list — no other edits) → **Save** → close the editor completely → reopen → **export**.
...
4. Required result, PRESERVED / exit code 0 with: **zero** deleted/added nodes, **zero** semantic node changes, **zero** lost or gained edges, **zero** new unintended terminals, and `oraya_bedroom_count` = `69114` / `custom_69114` bindings intact (also run `node scripts/validate-whatchimp-flow.mjs <re-export> --strict-binding` → exit 0; a missed redraw shows up as `single-parent-contract` hub-count errors, and an illegal extra Condition inbound edge as a `condition-inbound-limit` error, each pointing back to the checklist).
5. If the editor refuses ANY of the 39 connections, **stop and report immediately** — every item targets a User Input Flow wrapper or a Text using a pair the operator has already drawn successfully on this tenant, so a refusal is new platform evidence. Do not insert nodes to work around it.
6. TEST integrations only (sections A0/A1); **no production activation** and no production integration edits.
7. Save compatibility may not be claimed until this passes; only then proceed to section C.
...
3. Confirm the import persisted structurally. At this point the API nodes still display the **Production** integration names carried in the export — that is expected before rebinding:
   ...
   - The multi-input merge points arrive from import with **only their first inbound arrow** — that is the expected hybrid state, not a failure. **Draw the missing connections exactly per the regenerated [`V6_REDRAW_CHECKLIST.md`](V6_REDRAW_CHECKLIST.md), then save → close → reopen and confirm every drawn arrow survived.** If the editor refuses any listed connection (e.g. the infinite-loop warning), or a drawn arrow disappears after save, **stop and report immediately** — do not insert nodes to work around it (round trips #2 and #3 both halted on exactly such refusals; see `roundtrips/ROUNDTRIP_2_FINDINGS.md` and `roundtrips/ROUNDTRIP_3_FINDINGS.md`).
