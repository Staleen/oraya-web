 ...
 *
 * The two card schemes number these differently, and the set has to cover both
 * — Oraya does not know the scheme at this point and does not need to:
 *
 *   Visa / Amex / JCB / Diners / Discover   05 authenticated · 06 attempted · 07 none
 *   Mastercard Identity Check               02 authenticated · 01 attempted · 00 none
 *
 * Both zero-padded and bare forms appear depending on which field carried it
 * (`ucafCollectionIndicator` is documented as the last digit alone), so both
 * are listed. `07`/`7` is what Oraya saw live before 3-D Secure was requested
 * at all: no authentication happened.
 */
const AUTHENTICATED_ECI = new Set(["05", "5", "02", "2"]);
const ATTEMPTED_ECI = new Set(["06", "6", "01", "1"]);

export type PayerAuthenticationResult = {
  /**
   * The numeric ECI, whichever field this scheme returns it in — `eciRaw`
   * (all schemes), `ucafCollectionIndicator` (Mastercard) or `eci` (Visa
   * family, not-enrolled only). Never a commerce indicator.
   */
  eci?: string | null;
  /**
   * The authentication value: CAVV on the Visa family, AAV
   * (`ucafAuthenticationData`) on Mastercard. Empty means nothing was proven.
   */
  cavv?: string | null;
 ...
 *
 * WHICH FIELDS, AND WHY — this is scheme-specific, and reading the wrong ones
 * refused a genuinely authenticated live payment on 2026-08-13 (KNOWN_BUGS #35:
 * request 7866300639876438904889, Mastercard, Transaction Status Y, ECI 2, AAV
 * present, authorization reason 100 Success — voided by Oraya one second later).
 *
 * Per CyberSource's `PtsV2PaymentsPost201ResponseConsumerAuthenticationInformation`:
 *
 *   cavv                   "…for Visa, American Express, JCB, Diners Club, and
 *                           Discover transactions after the customer is
 *                           authenticated."      → ABSENT on Mastercard.
 *   ucafAuthenticationData "AAV is a unique identifier generated by the
 *                           card-issuing bank for Mastercard Identity Check
 *                           transactions after the customer is authenticated."
 *                                                → the Mastercard CAVV.
 *   eciRaw                 "ECI value that can be returned for Visa,
 *                           Mastercard, American Express, JCB, Diners Club, and
 *                           Discover. The field is absent when authentication
 *                           fails."              → the cross-scheme ECI, and
 *                                                  the one to prefer.
 *   ucafCollectionIndicator "Numeric ECI returned only for Mastercard Identity
 *                           Check transactions. Absent when authentication
 *                           fails."
 *   eci                    "…returned only for Visa, American Express, JCB,
 *                           Diners Club, and Discover transactions WHEN THE
 *                           CARD IS NOT ENROLLED."
 *
 * So the two fields this used to read — `eci` and `cavv` — are both
 * Visa-family, and `eci` is additionally a *not-enrolled* field. Between them
 * they cannot describe a successful Mastercard authentication at all, and `eci`
 * cannot describe a successful enrolled Visa one either.
 *
 * `ecommerceIndicator` is deliberately NOT part of the ECI chain any more. It
 * is documented as the "commerce indicator for cards not enrolled" and its
 * values are `internet | js_attempted | js_failure | spa | vbv_attempted |
 * vbv_failure` — words, never digits. It was being compared against numeric ECI
 * sets it can never match, so on a Mastercard it silently supplied `"spa"`
 * where an ECI was expected: not a fallback, a decoy.
 */
export function readPayerAuthenticationResult(payload: {
  ...
  consumerAuthenticationInformation?: {
    eci?: unknown;
    eciRaw?: unknown;
    ucafCollectionIndicator?: unknown;
    ecommerceIndicator?: unknown;
    cavv?: unknown;
    ucafAuthenticationData?: unknown;
    accessToken?: unknown;
    stepUpUrl?: unknown;
  ...
  // Numbers are accepted too: `ucafCollectionIndicator` is documented as
  // numeric, and JSON gives back a number, which `str` would have discarded.
  const val = (v: unknown): string | null =>
    typeof v === "number" && Number.isFinite(v) ? String(v) : str(v);
  return {
    // Cross-scheme first, then the Mastercard-only field, then the Visa-family
    // not-enrolled one. Every entry here is an ECI; none is a commerce indicator.
    eci: val(info?.eciRaw) ?? val(info?.ucafCollectionIndicator) ?? val(info?.eci),
    // The authentication value, under whichever name this scheme uses for it.
    cavv: str(info?.cavv) ?? str(info?.ucafAuthenticationData),
