...
Sub-phases **15A–15H** and **15I.1–15I.8** above record incremental work; they are not repeated here. **15I.9** and **15I.10** are active continuation items — see below.

### Phase 15I.9 — Adaptive booking flow (Instant vs Reserve UX)

- **Status:** OPEN — **UI/UX readiness only** for `/book` (Phase 15 continuing; does not re-close the Phase 15 umbrella).
...
- **Payment:** Online checkout / instant confirmation execution — **Phase 16**.

### Phase 15I.10 — Instant booking admin control

- **Status:** OPEN — **control layer only** (Phase 15 continuing).
- **Storage:** Per-villa flags in existing **`settings`** rows (`instant_booking_villa_mechmech`, `instant_booking_villa_byblos`); **no schema migration**.
- **Guest `/book`:** Instant UI appears only when the flag for the **selected villa** is enabled (defaults **on** when unset).
- **Payment:** Still **Phase 16** — toggles do not execute or persist payments.
