...
**Phase 15 critical regression/security fix:** `/book` reserve auto-advance now suppresses only after an explicit Back to Step 1 and resets when stay selection changes; public `/api/settings` is allowlisted to guest-safe keys only; `/api/members` requires same-user bearer auth; booking add-on audit errors fail closed before insert.

**Phase 15 medium regression hardening:** `/book` request review copy now stays request-safe even when instant eligibility signals are present; instant booking flags fail closed when unavailable; targeted admin actions safe-parse failures and surface visible errors while clearing busy state; event inquiry submit handles non-JSON errors cleanly.

**Phase 15 production settings security hotfix:** settings table lockdown is captured in `sql/phase-15-settings-rls-lockdown.sql`; browser reads use the allowlisted `/api/settings` route, and public testimonial reads are filtered to approved rows server-side before leaving the API.
...
**Phase 15 — `/events/inquiry` UX parity (guest flow):** **Mobile input zoom** fixed with the same **`.oraya-book-input-zoom-fix`** pattern as **`/book`**. **Guest contact validation** matches **`/book`** (full name required; **WhatsApp/phone or email** — at least one; email validated only when provided). **Validation scroll/focus** on **`goNext`**, **`handleSubmit`**, and **Add recommended setup** when event type is missing. **Micro-interaction** utilities aligned where controls were missing them. Helper export **`getEventServiceGroupForCatalogRow`** in **`lib/event-service-requirements.ts`** supports focusing the first service in a missing required group (UI only). **Information architecture:** long helper and trust copy moved into **mobile-safe info popovers** (same **`details` / `.book-info-panel`** pattern as **`/book`**); **optional notes** use a **collapsed Special requests** row; **Continue** on Step 2 sits **before** the **setup estimate** and **inquiry timing** prose. **Trust & conversion:** Step 3 adds a **What happens next** panel and **`EVENT_INQUIRY_*`** copy in **`lib/booking-trust-messaging.ts`** aligned with **stay** request tone (**inquiry ≠ confirmed booking**, **no payment at this stage**, **WhatsApp or email** follow-up); **WhatsApp** uses **`whatsapp_number`** with an **event-inquiry prefill**; **cancellation / changes** points to **`/legal/refund`** via the same link label as **stay** review (**no automatic refund implied**); **submit subline** and post-submit **not confirmed** line mirror **`STEP4_TRUST.request`** expectations without implying **instant booking**. **Final polish:** The event inquiry flow is **fully aligned with booking UX standards** for step progression, CTA affordances, and feedback; **conversion and interaction polish** (step clarity, disabled/loading submit guards, single-surface validation messaging, spacing/CTA parity with **`/book`**, safe-area padding) is **complete** — **no inquiry logic, API, or schema changes**.
