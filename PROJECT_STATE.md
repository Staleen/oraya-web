...
- **`data-theme="light"` | `data-theme="dark"`** on `<html>`; default SSR **`light`**; `beforeInteractive` script reads `localStorage` key `oraya-theme` when set to `light`/`dark`, else **first visit stays light** (no system preference)
...
- **Toggle:** top-right on homepage nav (with Reserve / account); top-right on `/book` (loading, auth gate, main flow); **light → moon (switch to dark), dark → sun (switch to light)**; `aria-label` + `title`; compact on narrow widths
