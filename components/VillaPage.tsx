...
/**
 * Remediation 5.5 — the single villa-page template. The former
 * app/villas/mechmech and app/villas/byblos pages were byte-identical apart
 * from this config (same pattern as the explore pages).
 */
export interface VillaPageConfig {
  /** Canonical villa label, e.g. "Villa Mechmech". */
  villa: "Villa Mechmech" | "Villa Byblos";
  /** Media API slug, e.g. "mechmech". */
  slug: string;
  heroGradient: string;
  locationLine: string;
...
export default function VillaPage({ config }: { config: VillaPageConfig }) {
  ...
  useEffect(() => {
    fetch(`/api/media?villa=${encodeURIComponent(config.slug)}`)
      .then((r) => r.json())
  ...
  const fromPrice = formatVillaFromPrice(config.villa, pricing);
  const instantHeroBadge = instantBookingEnabledForVilla(config.villa, instantBookingFlags);
  const bookHref = `/book?villa=${config.villa.replace(/ /g, "+")}`;
          ...
          <div style={{
            ...
            <div>
              ...
              <h1 style={{ fontFamily: PLAYFAIR, fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 400, color: HERO_TEXT, margin: 0, lineHeight: 1.15 }}>
                {config.villa}<br />
                <span style={{ fontStyle: "italic", opacity: 0.7 }}>{config.taglineItalic}</span>
              </h1>
              {fromPrice && (
      ...
      <section className="oraya-section-tone" style={{ backgroundColor: WHITE, padding: "5rem 3rem" }}>
        <div style={{ maxWidth: "760px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: PLAYFAIR, fontSize: "clamp(1.5rem, 3vw, 2.2rem)", fontWeight: 400, color: CHARCOAL, lineHeight: 1.25, marginBottom: "1.5rem" }}>
            {config.aboutHeading[0]}<br />{config.aboutHeading[1]}
          </h2>
      ...
      <section className="oraya-section-tone" style={{ backgroundColor: WHITE, padding: "3rem" }}>
        <div style={{ maxWidth: "760px", margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "1rem 2.5rem" }}>
            <a href={`/house-book/${config.slug}`} className="no-underline uppercase oraya-link-cta" style={{ fontFamily: LATO, fontSize: "11px", letterSpacing: "2px", color: GOLD, paddingBottom: "2px" }}>
              The House Book
            </a>
            <a href={`/explore/${config.slug}`} className="no-underline uppercase oraya-link-cta" style={{ fontFamily: LATO, fontSize: "11px", letterSpacing: "2px", color: GOLD, paddingBottom: "2px" }}>
              {config.exploreLabel}
            </a>
          </div>
        </div>
