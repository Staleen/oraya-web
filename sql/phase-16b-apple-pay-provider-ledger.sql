-- Phase 16B Apple Pay provider-ledger bridge.
-- This keeps Apple Pay independent from ordinary card readiness and records an
-- Apple-only authorization as an auditable wallet transaction.

create unique index if not exists payment_requests_one_active_apple_pay_collection
  on public.payment_requests (booking_id, purpose, amount, currency)
  where booking_id is not null
    and status in ('active', 'partially_paid')
    and allowed_methods @> array['apple_pay']::text[];
...
begin
  ...
  if found then
    if v_existing.payment_request_id is distinct from p_request_id
       ...
       or v_existing.provider_reference <> btrim(p_provider_reference)
       or v_existing.wallet_presentation is distinct from p_wallet_presentation then
  ...
  if p_wallet_presentation is null then
    if not ('card' = any(v_request.allowed_methods)) then
      raise exception using errcode = 'P0001', message = 'card_not_allowed';
    end if;
  elsif p_wallet_presentation = 'apple_pay' then
    if not ('apple_pay' = any(v_request.allowed_methods)) then
      raise exception using errcode = 'P0001', message = 'apple_pay_not_allowed';
    end if;
  else
    raise exception using errcode = 'P0001', message = 'wallet_not_allowed';
  end if;
  ...
  if v_request.booking_id is not null then
    ...
    update public.bookings
      set amount_paid = v_booking_paid,
          ...
          payment_method = case when p_wallet_presentation = 'apple_pay' then 'apple_pay' else 'card_manual' end,
