...
- [x] 1.3 **Cap stay length + reject past check-ins on `POST /api/bookings`** — done (`lib/booking-date-rules.ts`: `MAX_STAY_NIGHTS=60`, past check-in rejected with today allowed (UTC), wired into `/api/bookings` POST before the pricing engine and the member PATCH; 5 tests). Commit: see `Remediation 1.3`.
