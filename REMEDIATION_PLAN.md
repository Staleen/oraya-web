...
- [x] 1.6 **Webhook `set_paid` double-count** — done (`lib/payments/webhook-set-paid.ts` pure decision: any already-paid link is idempotent incl. different-reference-same-session; DB write guarded with NULL-safe not-paid filter + matched-row check, 0 rows → idempotent; 6 tests; KNOWN_BUGS #14 note added). Commit: see `Remediation 1.6`.
