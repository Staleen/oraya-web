...
- [x] 2.3 **Stop echoing raw DB errors on public routes** — done: all six call sites now log server-side and return generic messages (butler availability uses the route's snake_case error vocabulary). Commit: see `Remediation 2.3` = `f9c5c01`.
- [x] 2.4 **Quick-win sweep** — done, all nine sub-items in one commit:
  ...
  Commit: see `Remediation 2.4` = `31825f0`.
...
- [x] 6.2 Final full verification — done 2026-07-23: `tsc --noEmit` exit 0, `next build` exit 0, **252/252 tests** across 22 suites (baseline was 168/11), lint clean (only pre-existing `no-img-element` warnings), `npm audit` re-run: 5 findings remain, all inside `next@14.2.35` (resolved by the deferred Next 16 upgrade task). DECISIONS_LOG carries dated entries for Phases 1, 2, 3, 4, and 5-6; KNOWN_BUGS #14 annotated (1.6). Every plan box is checked (three sub-parts explicitly BLOCKED with reasons: 5.2 render-section memoization, 5.5 homepage server conversion + next/image heroes). PR opened from `claude/remediation-health-check-2026-vesyus`.
...
- [x] Schedule the Next 15+/React 19 upgrade task (4.3 default decision: separate scoped task; also resolves the 5 remaining `npm audit` findings that live inside next@14). Done: shipped as Plan 3 Phase 5 (PR #93) — Next 16.2.11 + React 19.2.8, npm audit 0 vulnerabilities.
