...
**Updated:** 2026-07-28 (Phase 16 row reconciled with merged PRs #85–#94)
...
## Phase 16 — In Progress (16A substantially complete; 16B active)

- **Status:** 🟡 in progress
- **Sub-phases:**
  - **16A** WhatsApp AI Butler — 🟡 in progress, **substantially complete**. Shipped: read-only foundation (`/api/butler/health|event-types|addons|availability|normalize-dates`); lead intake (`/api/butler/lead` + `whatsapp_leads` + `/admin/leads`); secure website handoff (`/api/butler/prefill` + `?h=…` on `/book`); lead → booking identity continuity (best-effort `whatsapp_leads.linked_booking_id` writer on `/api/bookings` POST); WhatsApp identity v2 (`/api/butler/identify` — subscriber-id primary continuity key, `chat_id` diagnostic-only, email-or-name identity proof, booking-view URL enrichment on identity-established active branches); confirmed-guest info boundary (`/api/butler/confirmed-guest-info` — narrow Phase 16A allow-list, refuses pending / cancelled / unverified); Butler namespace standardization (`/api/butler/*`); inbound-message convenience (`message_text` + bounded 8-char hex extractor in [lib/butler/extract-booking-reference.ts](../../lib/butler/extract-booking-reference.ts), forward-compatible for non-WhatChimp channels); website CTA prefill routing (plain human sentences `"Check my booking <ref>"` / `"Help with my booking <ref>"` — the brief 2026-05-23 marker scheme was withdrawn on 2026-06-03). **Outstanding (operator-side / policy-side):** WhatChimp confirmed-guest flow wiring, confirmed-guest routing refinement, generic AI containment for booking-sensitive confirmed-guest questions, `POST /api/butler/flow-submit` if/when WhatsApp-side booking submission is scoped in. **Smart-lock PIN / access-code / exact-location automation is NOT Phase 16A** — it remains Phase 16D.
  - **16B** Payment processing + refunds — 🟡 **in progress, active**. Shipped: provider-agnostic hosted-checkout architecture; payment-link columns on `bookings` (16B.1 migration applied; 16B.4 Credit Libanais provider-compat constraint widening migration staged in [sql/phase-16b4-credit-libanais-provider-compat.sql](../../sql/phase-16b4-credit-libanais-provider-compat.sql)); `POST /api/payments/checkout` booking-first hosted-checkout creation; `POST /api/payments/webhook/[provider]` verification-authoritative reconciliation surface; admin payment settings (mode, deposit minimum, manual rails, guest copy, provider display name); `/admin/settings` non-secret readiness summary via `/api/payments/readiness`; three-step `/book` flow with dual-CTA Step 3 (primary "Continue to secure payment", secondary "Reserve now, pay later"); add-ons / special requests no longer block stay payment. **Production direction:** Credit Libanais / NetCommerce / MPGS — only approved production provider. **Not production direction:** Stripe — retained adapter is isolated to local / dev (`PAYMENT_PROVIDER=stripe` is rejected in production). **Shipped 2026-07-02 (PR #64):** NetCommerce/CyberSource Unified Checkout sandbox path. **Shipped 2026-07-24 (Plans 3–4, PRs #91/#92/#94):** durable `payment_attempts` idempotency ledger (KNOWN_BUGS #14 resolved); fail-closed webhook/MLE verification with authoritative attempt reconciliation; `/api/health` payment-attempt visibility; manual-first refund recording + [REFUND_RUNBOOK.md](../system/REFUND_RUNBOOK.md) (KNOWN_BUGS #15 resolved-by-policy); fail-closed `payments_live_enabled` rollout switch + admin toggle + readiness panel. **Outstanding:** live production activation (external NetCommerce credentials + operator go-live checklist in the PR #94 body), declined-card validation (external vector, KNOWN_BUGS #9), settlement/report reconciliation, payment email lifecycle validation, refunds automation (separate later plan), WhatsApp payment-status replies, Instant Book payment execution. See [/docs/phases/PHASE_16B_PLAN.md](PHASE_16B_PLAN.md) and [/docs/system/PHASE_16B_PAYMENT_OPERATIONS_AUDIT.md](../system/PHASE_16B_PAYMENT_OPERATIONS_AUDIT.md) (2026-07-28 addendum).
