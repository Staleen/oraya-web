...
(async () => {
  ...
  for (const pair of [['byblos','oraya-home-book-byblos.html'],['mechmech','oraya-home-book-mechmech.html']]) {
    ...
    for (const id of ['p2','p3','p4']) {
      ...
      if (!el) { console.log('MISSING: '+pair[0]+'-'+id); continue; }
      const box = await el.boundingBox();
      await pg.screenshot({ path: path.join(DIR,'exports',pair[0]+'-'+id+'-check.png'), clip: { x:box.x, y:box.y, width:box.width, height:box.height } });
      console.log('OK: '+pair[0]+'-'+id);
    }
  }
  await browser.close();
