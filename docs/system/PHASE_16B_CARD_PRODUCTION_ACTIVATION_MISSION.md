...
**First attempt evidence (2026-08-10):** with the live switch opened for the approved window, a $1.00 standalone card request failed CLIENT-SIDE before any card entry: the capture context was minted with `targetOrigins: ["https://stayoraya.com"]` (canonical bare origin) while the checkout page serves on `https://www.stayoraya.com`, so Unified Checkout refused initialization ("secure checkout failed"; session `oraya_e1c57dd2…` created, zero attempts claimed, zero charges).

**Second attempt evidence (2026-08-10, supersedes the first correction):** listing BOTH host variants (merged PR #124) also fails — the deployed v2 client (`VAS.UnifiedCheckout`) throws `UnifiedCheckoutError UNUSED_TARGET_ORIGINS ("One or more target origins are unused", details: ["https://stayoraya.com"])`, captured live in-page after decoding the deployed capture context and initializing manually. Unified Checkout requires targetOrigins to contain EXACTLY the origins in use. Final fix: the capture context is minted for the single origin the guest's request actually arrives on, trusted only within the canonical www/bare family with fallback to the canonical origin ([lib/payments/target-origin.ts](../../lib/payments/target-origin.ts) `resolveEffectiveCheckoutOrigin`, used by [lib/payments/request-origin.ts](../../lib/payments/request-origin.ts)); `targetOrigins` is a single exact origin again. The controlled window re-runs after this deploys.
