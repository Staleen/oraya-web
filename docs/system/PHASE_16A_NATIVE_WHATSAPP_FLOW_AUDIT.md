...
**Date:** 2026-07-30 (outcome lines updated 2026-08-02; body preserved as the pre-cutover audit)

**Scope:** documentation/read-only audit only

**Outcome (2026-08-02):** **SHIPPED AND VERIFIED.** The external verification this audit gated on was completed, and the native WhatsApp Flow **"Stay Request"** (WhatChimp Flow ID `40377`) became the production stay intake on **2026-07-31**, verified end-to-end with test bookings `6ED26663` (keyword path) and `7D5C4BCD` (menu path) on 2026-07-31 and with real guest traffic on 2026-08-01. v6 is retained intact (triggers cleared) as the immediate rollback path. The published Flow's canonical JSON and the supporting WhatChimp exports are committed at [`artifacts/whatchimp/native-flow/`](../../artifacts/whatchimp/native-flow/); the shipped Flow is the single-terminal-screen v4 JSON there, not the two-screen draft proposed below. See [DECISIONS_LOG.md](DECISIONS_LOG.md) entries 2026-07-31 and 2026-08-02.

**Recommendation (pre-cutover, 2026-07-30 — since satisfied; see Outcome above):** **Proceed after external verification.** The Oraya lead contract can ingest the requested stay fields without a backend or schema change, and it already returns the secure `prefill_url`. The smallest production path is nevertheless gated on two WhatChimp tenant capabilities that the repository and public documentation do not prove: attaching/launching the existing Meta-owned Flow ID, and forwarding the complete submission to the existing authenticated Lead Submit integration while retaining `prefill_url` for the next WhatsApp reply.
...
**Proceed after external verification.** *(Status 2026-08-02: verification completed and the cutover shipped 2026-07-31 — see the Outcome line at the top of this document.)*
