...
3. ~~**Team** screen~~ — built 2026-08-07, with the accept-invite flow (`/ops-invite/[token]` + `POST /api/ops/invite/accept`). Invite delivery is deliberately **link-only** (owner copies the one-time link and sends it by WhatsApp); a Resend invite email remains future work. Later the same day: **re-invite** (fresh one-time link for a lost/expired invite, guarded on `password_hash IS NULL`) and the prototype's operator-capabilities panel.
4. ~~**Availability**~~ — built 2026-08-07 as read-only occupancy + feed freshness. ~~The owner screens: **Pricing, Extras, Payments**~~ — built 2026-08-07 on branch `claude/ops-owner-screens`, prototype-faithful (Live-now banner, pending-changes bar naming every unsaved edit, strike-through remove with "Keep it"). Owner-only API surface: `GET /api/ops/setup` + `PUT /api/ops/setup/{pricing,addons,payments}` (`requireOps({requiredRole:"owner"})`). Guards carried over and extended: the R-2 wipe guard on add-ons; compare-and-set on the pricing and payment-settings blobs (the S-8 lesson — a stale editor gets 409, not a silent overwrite); STRICT server-side pricing validation (the shared lenient parser falls back to defaults and must never gate a write) plus the shared `validatePricing` error gate; the add-ons write round-trips untouched operational fields (categories, villas, prep times) and overlays only `requires_approval`, and reports a second-phase failure explicitly as partial (R-6 lesson). **Honest deviations from the prototype:** no cleaning-fee field (the pricing engine has none) and minimum nights is per villa + per season, not per rate row. **The fail-closed live-payments switch is surfaced read-only** — its only writer remains the password-confirmed `/api/admin/payments/live-toggle` (DECISIONS_LOG 2026-07-25); the Extras screen's deeper operational rules stay in the legacy admin for now.
...
7. ~~**Event enquiry handling in /ops**~~ — built 2026-08-07: proposal draft/send (Phase 15H fields + the existing proposal email; an **accepted** proposal is refused for editing per audit B-11, and B-12 blocks sending an expired validity date) and event approval gated on `proposal_status === "accepted"` with the Phase 14J setup-day conflict check. Queue kinds `event_proposal_needed` / `event_accepted_unconfirmed` surface them on Today. **Still open:** converting an existing stay into an event (owner decision — add-ons on the stay vs a separate event enquiry), and event lead conversion from Enquiries (WhatsApp event leads are pointed at `/events/inquiry`).
8. **Asking for money** — built 2026-08-07 as 16B continuity: `request_deposit`, `send_reminder`, and a `record_payment` that now advances the lifecycle (`payment_status`, `payment_stage`, `amount_total/due` via `lib/payment-foundation.ts`) and sends the same three payment emails the admin sends, each behind a rendered preview and race-guarded on the lifecycle value shown. /ops asks and records; the 16B provider layer remains the only thing that charges a card.
