# Current Phase - Phase 16A closeout / Phase 16B hosted payment provider readiness

**Updated:** 2026-05-23
**Status:** Phase 16A WhatsApp / WhatChimp / Butler lead capture + secure website handoff + identity continuity + identity orchestration + confirmed-guest info boundary remain shipped. Phase 16A.2 `flow-submit` write-capable booking adapter remains outstanding. Phase 16B is now live through the settings-driven Reserve payment foundation: payment-link schema, runtime plumbing, admin payment settings, premium Step 3 decision UX, provider-agnostic hosted checkout architecture, and verified hosted-payment callbacks. Credit Libanais / MPGS is the only approved production provider path; Stripe remains isolated to local/dev testing only. Credit Libanais execution itself is still placeholder-only pending the bank's real hosted-checkout and callback contract.
...
- Returning-guest identity now runs through `/api/butler/identify` with WhatChimp `subscriber_id` as the primary continuity key, phone as a future-channel fallback, and booking reference + email/full-name proof as the explicit verification gate.
- `/api/butler/confirmed-guest-info` is the current confirmed-stay information boundary: it reuses identity orchestration, requires `status = confirmed`, and returns only booking reference, villa, dates, signed booking-view URL, operator-managed check-in guidance placeholder/text, and an access/location safety note.

The remaining Phase 16A scope is `POST /api/butler/flow-submit` - the write-capable booking adapter that turns a WhatsApp Flow submission directly into an Oraya booking row through the locked `/api/bookings` POST contract. No schema changes unless separately approved. No locked-API behavior changes. Idempotency details still need design confirmation before implementation.
...
- **Phase 16A - confirmed-guest info boundary (2026-05-23 merge, PR #43).** Added `POST /api/butler/confirmed-guest-info` as the strict surface for identified guests whose booking is already `confirmed`. It reuses the identity orchestrator, refuses pending / cancelled / unverified / ambiguous states, and returns only the Phase 16A confirmed-guest allow-list: public booking reference, villa, date-only check-in/check-out strings, signed booking-view URL, operator-managed `butler_checkin_guidance` (or a placeholder), and a fixed location/access safety note. It does not mutate bookings/leads/settings, send email, surface payment details, return exact location, or issue smart-lock/gate credentials. See [BUTLER_PLAYBOOK.md](BUTLER_PLAYBOOK.md) "Confirmed-guest info boundary".
...
- **Vercel env and SQL application status need human confirmation.** Repo docs and placeholders list the current env contract, but this audit cannot verify the Vercel panel or whether human-gated SQL helpers have been applied in Supabase. Production and Preview need the relevant Butler and payment env values set (Sensitive where applicable), and the Phase 16A.3 / 16B.4 SQL helpers applied when those features are expected to be fully active. See [KNOWN_BUGS.md](KNOWN_BUGS.md) #4.
...
4. **ChatGPT Project source refresh:** after this documentation PR is reviewed and merged, re-upload the refreshed source docs to the ChatGPT Project so new orchestrator chats stop starting from the May 9 state.
5. **16A.2 implementation (next coding session):** design and ship `POST /api/butler/flow-submit` per "Active objective" above without bypassing the locked `/api/bookings` authority.
