...
- **Phase 16A - Butler website handoff date/auth persistence hardening (this commit).** `normalizeLeadInput` now accepts plain `check_in` / `check_out` aliases in addition to the normalized WhatChimp fields when persisting `whatsapp_leads`, so the secure `/book?h=...` handoff can hydrate stay dates even when the upstream payload uses the shorter keys. On the `/book` client, Butler date hydration is now queued into the exact `dateRange` state the calendar uses and is re-applied after the auth gate resolves, so late villa resets or the guest/member gate cannot drop the decoded stay dates. The `/book` auth gate also preserves the full current booking URL in its member sign-in redirect, so the signed handoff token survives the login round-trip instead of depending on session timing. No schema changes. No locked API behavior changes.
