...
- **Phase 16A.2.f — WhatsApp lead → website prefilled booking handoff (this commit).** Existing `whatsapp_leads` rows now serve as the source of truth for a safe website handoff. `POST /api/butler/lead` still inserts one lead row, keeps its existing `{ ok, lead_id, message }` response fields, and additively returns `prefill_url` (`{SITE_URL}/book?h=<token>`). The token is a 2-hour HMAC-SHA256 opaque token signed with the new server-only `BUTLER_PREFILL_SECRET`; it carries only `lead_id`, `exp`, `jti`, `v:1`, and `purpose:"prefill"`. New `GET /api/butler/prefill?h=<token>` verifies the token (no `X-Butler-Secret`) and returns only `villa`, `check_in`, `check_out`, `sleeping_guests`, `full_name`, and `source` from the existing lead row. `/book?h=...` hydrates only empty/current-safe fields and strips `h` from the URL after success or failure. **No schema changes, no raw booking data or PII in URLs, no booking creation, no locked API touches.**
- **Phase 16A.2.e — WhatsApp lead persistence + admin lead dashboard (this commit).** Added a new `whatsapp_leads` Supabase table (schema in [/sql/phase-16a2e-whatsapp-leads.sql](../../sql/phase-16a2e-whatsapp-leads.sql)) and the surfaces that read and write it:
  ...
  - **No booking creation, no availability check, no DB writes outside `whatsapp_leads`, no email sends, no payment, no access details, no smart-lock, no new dependency.** Lead intake only — operators triage from `/admin/leads`. `BUTLER_WEBHOOK_SECRET` and `X-Butler-Secret` are reused unchanged for ingest; website prefill links use the dedicated `BUTLER_PREFILL_SECRET`. Active sub-phase remains `flow-submit`; this is the operational backbone the future `flow-submit` will hand off to.
