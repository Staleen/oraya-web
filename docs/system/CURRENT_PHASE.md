...
- `POST /api/payments/create-session` creates a hosted checkout session only after the locked `/api/bookings` POST successfully creates the booking row, via the existing checkout handler
...
1. **Hosted-checkout execution hardening.** Ensure `POST /api/payments/create-session` / the existing checkout handler and verified provider callbacks remain the only payment execution authority for Reserve bookings after the booking row exists.
