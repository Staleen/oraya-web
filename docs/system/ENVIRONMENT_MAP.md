...
- **Required:** local no · preview **no — keep unset (fail closed; Preview shares the production database)** · production yes, but **only at activation**: Meta has approved `oraya_arrival_guide_confirmed`, WhatChimp has the template synced with `#!variablename!#` variable linkage, and the WhatChimp Webhook Workflow "Confirmed booking" exists (all verified 2026-08-01 — the Production URL is the remaining on-switch).
...
- **Required:** no for card checkout. Set in Preview only after the merchant is enrolled for Apple Pay in CyberSource/NetCommerce, the exact preview test host is registered where required, and an Apple sandbox device is ready. Set in Production only after the host that actually serves checkout is verified and the full Apple Pay payment/webhook/refund path passes. The canonical guest URL `https://stayoraya.com` currently redirects to `https://www.stayoraya.com`; therefore the effective checkout host is currently `www.stayoraya.com`. If Vercel routing changes, re-verify the new effective host before leaving the flag enabled.
