...
## 2026-08-10 - Phase 16D Stage A: dark access-PIN magazine foundation (schema text, dedicated vault, memorable generator — no activation)
...
2. **Keyed HMAC fingerprint instead of the payment-ledger lookup hash.** A 256-bit pay token tolerates an unsalted SHA-256 lookup hash; a six-digit PIN (~20 bits) does not. `pin_fingerprint` is HMAC-SHA256 under the dedicated stable `ACCESS_PIN_FINGERPRINT_KEY`, globally unique, used only for duplicate prevention — never for lookup.
...
4. **The lifecycle is a database trigger, not app discipline.** `installing → available → disclosed → quarantined → deletion_pending → destroyed`; `available` requires BOTH per-lock install confirmations (A100 scheduled PIN; U100 scheduled user + PIN — two different manual workflows, confirmed separately); `disclosed` has no edge back to `available` (disclosure is permanent consumption); `destroyed` requires deletion confirmation from every lock that was actually installed; identity columns frozen; confirmation facts write-once; booking linkage set-once at disclosure with a DB-side property↔villa match. `reserved` is deliberately omitted — Stage C allocation will be a single-transaction `available → disclosed` CAS under an advisory lock, so a hold state would only add invalid paths.
5. **Interrupted loading never silently resolves.** An abandoned `installing` row stays incomplete (it cannot reach `available` without both confirmations) and keeps decryptable ciphertext so Stage B can re-reveal it — but only through an audited `pin_revealed_for_loading` event; the alternative exit is quarantine → per-lock deletion (only for locks actually reached) → destroy.
6. **Dedicated env keys with a rotating keyring.** `ACCESS_PIN_VAULT_KEYS` (ordered `k1:<base64-32>` list; first entry encrypts, all decrypt; envelopes `apv1.<kid>.<iv>.<tag>.<ct>` embed the key id so rotation = prepend) and `ACCESS_PIN_FINGERPRINT_KEY` (stable, never casually rotated). Deliberately not `ADMIN_SECRET` — the payment-token precedent of deriving from `ADMIN_SECRET` couples web-session rotation to ciphertext survival, and door credentials must not inherit that coupling. Both variables are **unset everywhere**; the vault fails closed.
7. **Generator policy.** CSPRNG (`crypto.randomInt`), exactly six digits, **leading zero rejected** because the workflow is a human manually copying the PIN into two Aqara devices and any numeric intermediate silently drops a leading zero (and this task could not verify both firmwares preserve it). Broad rejection families (runs/overuse, sequences, repeated halves/alternation/palindromes, common PINs, keypad lines, year/date shapes, context digits such as phones/booking references, historical fingerprints) plus varied multi-feature memorability scoring — grouping like `25-56-55` happens at display time, never as a generation template. Bounded attempts throw instead of looping.
...
**Impact:** new `sql/phase-16d-access-magazine.sql` (human-run, NOT executed), `lib/access/pin-vault.ts` + `pin-generator.ts` + `pin-lifecycle.ts`, four focused test files (50 tests incl. static assertions pinning the migration's security posture), `.env.example` placeholders, ENVIRONMENT_MAP/PROJECT_STATE/CURRENT_PHASE/ARCHITECTURE updates. No existing table, route, or locked surface touched. Human actions still required, in order: review + run the migration; generate and set the two Vercel secrets (Stage B); approve Stage B (ops console + 3-PIN physical pilot); approve the access-eligibility policy before Stage C (payment still never auto-confirms a booking).
