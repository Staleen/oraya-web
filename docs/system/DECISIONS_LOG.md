...
## 2026-08-10 - Phase 16D Stage A: dark access-PIN magazine foundation (schema text, dedicated vault, memorable generator — no activation; revised same day per PR #120 review)

**Decision:** Phase 16D begins with a foundation that cannot touch production: one human-run **unapplied** migration ([sql/phase-16d-access-magazine.sql](../../sql/phase-16d-access-magazine.sql)) and three server-only modules ([lib/access/pin-vault.ts](../../lib/access/pin-vault.ts), [lib/access/pin-generator.ts](../../lib/access/pin-generator.ts), [lib/access/pin-lifecycle.ts](../../lib/access/pin-lifecycle.ts)) with no runtime consumer. The load-bearing choices:

1. **Dedicated tables, not `settings` or `bookings` jsonb.** The generic admin settings surface upserts arbitrary keys and returns every non-protected row, and booking-jsonb enrichment is contractually advisory-only and guest-invisible — both categorically wrong for physical-access credentials. `access_credentials` + append-only `access_credential_events` follow the proven payment-ledger posture (RLS with no client policies, revoke anon/authenticated, service-role grants with **no delete**, raising immutability trigger, pinned `search_path`).
2. **Keyed, VERSIONED HMAC fingerprints instead of the payment-ledger lookup hash.** A 256-bit pay token tolerates an unsalted SHA-256 lookup hash; a six-digit PIN (~20 bits) does not. `pin_fingerprint` is HMAC-SHA256 under the active entry of the versioned `ACCESS_PIN_FINGERPRINT_KEYS` keyring, with the producing `pin_fingerprint_key_id` stored per row; duplicate detection fingerprints every candidate under **every retained key version** (`fingerprintAccessPinAllKeys`), so rotation never breaks historical duplicate detection. Used only for duplicate prevention — never for lookup. Emergency-compromise behavior (a leaked fingerprint key + DB read = treat active PINs as disclosed) is documented in ENVIRONMENT_MAP.
3. **Ciphertext erasure at destruction is a CHECK constraint, not a convention.** `(status = 'destroyed') = (pin_ciphertext is null)`: a destroyed credential cannot stay decryptable, a live one cannot lose its ciphertext, and the retained fingerprint row plus global uniqueness means that numeric PIN can never be generated again.
4. **The lifecycle is a database trigger, not app discipline.** `installing → available → disclosed → quarantined → deletion_pending → destroyed`; every per-lock confirmation is a write-once **timestamp + actor pair** (four pair CHECKs — the actor half can never bypass a state guard); `available` requires BOTH complete install pairs (A100 scheduled PIN; U100 scheduled user + PIN — two different manual workflows, confirmed separately); `disclosed` has no edge back to `available` (disclosure is permanent consumption); `destroyed` requires the complete deletion pair for every lock that was actually installed; comprehensive state/fact-consistency CHECKs bind disclosure facts (all-or-none, absent before disclosure), quarantine facts (pair, exactly from quarantine onward), deletion facts (only from quarantine onward), and `destroyed_at` to their proper states; identity columns frozen; booking linkage set-once at disclosure with a DB-side property↔villa match; replacement lineage verified **against the predecessor row in-database** (no self-reference; same property and booking; predecessor quarantined/deletion_pending/destroyed; backup must supersede the primary, replacement any consumed predecessor). `reserved` is deliberately omitted — Stage C allocation will be a single-transaction `available → disclosed` CAS under an advisory lock, so a hold state would only add invalid paths.
5. **Interrupted loading never silently resolves, and no mutation escapes the audit ledger.** An abandoned `installing` row stays incomplete (it cannot reach `available` without both complete pairs) and keeps decryptable ciphertext so Stage B can re-reveal it — but only through an audited `pin_revealed_for_loading` event. Every row MUTATION (generation, each per-lock confirmation, every status change) gets its immutable audit event inserted **transactionally by an AFTER trigger**, so even a direct service-role UPDATE cannot bypass the ledger. Audit rows carry allow-listed reason codes only — the free-text `reason`/`note` surface was removed so no audit field can accidentally persist a PIN.
6. **Dedicated env keys, both as rotating keyrings.** `ACCESS_PIN_VAULT_KEYS` (ordered `k1:<base64-32>` list; first entry encrypts, all decrypt; envelopes `apv1.<kid>.<iv>.<tag>.<ct>` embed the key id so rotation = prepend) and `ACCESS_PIN_FINGERPRINT_KEYS` (same format; prepend to rotate; never drop an entry that stored fingerprints still reference). Deliberately not `ADMIN_SECRET` — the payment-token precedent of deriving from `ADMIN_SECRET` couples web-session rotation to ciphertext survival, and door credentials must not inherit that coupling. Both variables are **unset everywhere**; the vault fails closed and carries a runtime browser-context guard (the repo has no compile-time `server-only` package).
7. **Generator policy.** CSPRNG (`crypto.randomInt`), exactly six digits, **leading zero rejected** because the workflow is a human manually copying the PIN into two Aqara devices and any numeric intermediate silently drops a leading zero (and this task could not verify both firmwares preserve it). Broad rejection families (runs/overuse, sequences, repeated halves/alternation/palindromes, common PINs, keypad lines, year/date shapes, context digits such as phones/booking references, historical fingerprints under every retained key version) plus varied multi-feature memorability scoring — grouping like `25-56-55` happens at display time, never as a generation template. Bounded attempts throw instead of looping. A deterministic full-enumeration test pins the accepted population (~336k of 900k candidates, ≥15 distinct feature shapes, no shape above 35%) so future tuning cannot silently collapse the search space into one template.

**Reason:** the 2026-08-09 read-only audit identified these as the smallest safe primitives that make every later stage (ops console, allocation, guest reveal, Aqara monitoring) an additive consumer instead of a schema redesign — while keeping Stage A physically incapable of touching guests, bookings, payments, or locks.

**Impact:** new `sql/phase-16d-access-magazine.sql` (human-run, NOT executed), `lib/access/pin-vault.ts` + `pin-generator.ts` + `pin-lifecycle.ts`, four focused test files (57 tests incl. static assertions pinning the migration's security posture and a deterministic full-enumeration population test), `.env.example` placeholders, ENVIRONMENT_MAP/PROJECT_STATE/CURRENT_PHASE/ARCHITECTURE updates. No existing table, route, or locked surface touched. Human actions still required, in order: review + run the migration; generate and set the two Vercel secrets (Stage B); approve Stage B (ops console + 3-PIN physical pilot); approve the access-eligibility policy before Stage C (payment still never auto-confirms a booking).

**Reversible?:** yes — nothing is applied or consumed; deleting the files reverts Stage A completely.
