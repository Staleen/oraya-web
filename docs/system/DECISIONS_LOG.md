...
- **Pinned operator export (post-mapping, production-verified backup):** `artifacts/whatchimp/roundtrips/Oraya_natural_intake_v6.production-locked.operator-export.txt`, SHA-256 `C80F325D1EFF7D81D09F7C9ADD33486187A62D31FD1CA67F4E850A10CDE076E9`, 177,262 bytes, 192 nodes. Scanned clean — no `X-Butler-Secret`/URLs/bodies (the flow export carries integration IDs only; secrets live outside the export). **Known tenant-side deviation (documented, NOT a defect):** in David's live bot 11 of the 12 Lead Submit nodes are bound to `7459` and 1 to `6961` (the canonical artifact keeps all 12 on `6961`). Both are the "Oraya Lead Submit" class POSTing to the same `/api/butler/lead` endpoint, and live verification confirmed the lead saved and the secure link minted — so the deviation is operational. The repo canonical stays on `6961`; this pinned file records the operator's actual approved bot as-run.
...
**Impact:** docs only — `docs/system/CURRENT_PHASE.md` (status → LOCKED), `docs/system/KNOWN_BUGS.md` #10 (marked RESOLVED), this entry, and the pinned operator export. **No code, no artifact regeneration, no flow-layout change, no API-URL change, no secret/`.env` change, no production WhatChimp change.**
