...
The real WhatChimp custom-field id for `oraya_bedroom_count` did not exist when this entry was written; the artifact initially shipped with a documented placeholder (a fabricated id is never acceptable). Superseded on this same branch by the 2026-07-03 entry above: the operator created field `69114` and the generator now emits it directly — the canonical artifact ships fully bound. The artifact is an **import-ready v6 candidate** — the authenticated WhatChimp import/save/re-export round trip (validated with `--strict-binding`) and the live scenario checklist (`artifacts/whatchimp/V6_ROUNDTRIP_CHECKLIST.md`) remain the production gate. WhatChimp HTTP API integrations are tenant-global shared objects: production integrations `7466`/`8101`/`6961`/`7459` must not be edited for testing — pre-cutover testing uses cloned TEST integrations pointed at the PR's Vercel Preview deployment (checklist section A0/A1).
