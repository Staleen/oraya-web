...
**Implementation checkpoint, 2026-08-09:** PR #110 shipped F1/F2 and the core F3 link path; PR #112 shipped CyberSource webhook JWE, separate timestamped signature verification, and replay claiming; PR #113 shipped the canonical booking/standalone card bridge; PR #114 shipped provider-gated Apple Pay capture context plus the operations reconciliation/settlement view; PR #116 shipped CyberSource-compatible GET+POST health checks and the evidence-gated card-activation mission. Production card, Payments MLE, and webhook credentials are installed server-side. A no-card production capture-context smoke returned HTTP 201, both health-check methods return HTTP 200, and CyberSource accepted exactly one production Unified Checkout subscription whose current status is `PENDING_REVIEW`. The live schema supports request-scoped attempts and an atomic provider-payment RPC. `/ops/payments` exposes provider readiness, reconciliation attention, merchant references, and gross/fee/net ledger totals. Apple Pay support exists behind a separate exact flag and records as a wallet only on an Apple-only request; it cannot activate until merchant enrollment, verification of the effective checkout host (currently `www.stayoraya.com` after the bare-domain redirect), and device testing are proven. Subscription activation, verified provider delivery, controlled real-card/decline evidence, native-wallet contracts, and TMS/legal approval remain external gates.
...
The provider-independent payment operating system is implemented. Remaining work cannot be truthfully completed without external artifacts: activation and delivered-provider proof for the configured card/webhook credentials, controlled card/decline/refund/settlement evidence, Apple Pay enrollment/domain/device proof, official Whish/OMT/Suyool merchant contracts, and CyberSource TMS plus legal/consent approval. The `bookings.amount_*` and `bookings.payment_*` fields are compatibility summaries; the immutable transaction ledger is the durable money history.
