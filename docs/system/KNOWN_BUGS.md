...
- **Recommended fix path:** (1) operator runs the drawability probe matrix in [ROUNDTRIP_2_FINDINGS.md §6](../../artifacts/whatchimp/roundtrips/ROUNDTRIP_2_FINDINGS.md) on the disposable bot; (2) record proven/rejected pairs in `scripts/whatchimp/natural-intake-profile.json`; (3) human decision on the rebuild direction (Condition-clone cascade if probes pass; otherwise quantified behavior reduction or approved UX change); (4) rebuild generator, regenerate candidate + checklist, then the full round-trip procedure in [V6_ROUNDTRIP_CHECKLIST.md](../../artifacts/whatchimp/V6_ROUNDTRIP_CHECKLIST.md).
