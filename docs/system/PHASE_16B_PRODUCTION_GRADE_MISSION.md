...
- ~~Still open from that audit:~~ **FOUR OF FIVE CLOSED 2026-08-12** (branch `claude/book-selection-truth`): a villa change now keeps the guest's dates and re-checks them against the new villa, saying so when they cannot be kept · a half-finished range no longer survives leaving the calendar, and while pending it says which date is held and what the next click does · the instant promise is gated · an expired payment link now carries the WhatsApp CTA. **Still open: a failed checkout session cannot be told from a standalone payment request** (KNOWN_BUGS #27.4) — deliberately, because fixing it means keying the title on `isPaymentRequest`, which commit `43b1db4` abandoned.
- ~~Still open, same defect as the fixed email:~~ **CLOSED 2026-08-12.** `lib/send-booking-pending-email.ts` (sent on **every** booking creation) and the booking view's "Your note" card both route through `lib/guest-visible-note.ts`. KNOWN_BUGS #26 is closed on all three surfaces.
- The instant lane on `/book` advertises speed that only exists once W3 is switched on. **The advertisement is gone** — the line is now gated on the master switch, the villa flag and stay eligibility, and stays silent while the switch is unreadable from the client. **Note for W3:** switching instant booking on will NOT bring the line back on its own — `instant_booking_auto_confirm` is missing from the `/api/settings` public allow-list, so `/book` still cannot see it. One line, tracked as KNOWN_BUGS #30.1, and it belongs in the W3 switch-on checklist.
