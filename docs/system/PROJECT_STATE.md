...
- **Phase 16A - IN PROGRESS.** WhatsApp AI Butler read-only foundation (`/api/butler/health|event-types|addons|availability|normalize-dates`), lead intake (`/api/butler/lead` + `whatsapp_leads` + `/admin/leads`), secure website handoff (`/api/butler/prefill` + `?h=...` on `/book`), lead -> booking identity continuity (best-effort `whatsapp_leads.linked_booking_id` writer in `/api/bookings` POST), identity orchestration (`/api/butler/booking-lookup`, `/api/butler/identify`), signed booking-view URL surfacing on identity-established active bookings, and confirmed-guest info boundary (`/api/butler/confirmed-guest-info`) all shipped. Outstanding 16A scope: `POST /api/butler/flow-submit` (write-capable booking adapter), WhatChimp-side subscriber-id wiring confirmation for the booking-request flow, human-escalation routing, and AI prompt tuning. The 8-character booking reference on `/booking/view/[token]` is a **public support code, not an access PIN** - access credentials remain Phase 16D.
- **Phase 16B - IN PROGRESS.** Phase 16B.1 payment-link schema/domain scaffolding and 16B.2 runtime plumbing are complete. Phase 16B now uses a provider-agnostic hosted checkout architecture on the Reserve path: `/book` still creates the booking first, `POST /api/payments/checkout` resolves the configured hosted-payment adapter, persists booking-side `payment_link_*` state only for providers supported by the current schema, and verified callbacks reconcile payment state through `POST /api/payments/webhook/[provider]`. Admin payment settings and the Step 3 two-path payment decision UX are now settings-driven, so operations can control request-only / manual / online / hybrid guest behavior before the real Credit Libanais / MPGS adapter is implemented. Credit Libanais / MPGS is the only approved production provider path; the retained Stripe adapter is isolated to local/dev testing only. Refunds, WhatsApp payment replies, and Instant Book execution remain later 16B work. See [/docs/phases/PHASE_16B_PLAN.md](../phases/PHASE_16B_PLAN.md) and [/PHASE_16_PLAN.md](../../PHASE_16_PLAN.md).
...
- **Booking flow** - public stay booking via [app/book/page.tsx](../../app/book/page.tsx); Reserve (request -> hosted provider checkout -> webhook-driven payment state) and Instant Book (UI only today). Server-side overlap protection and booking creation authority remain in [app/api/bookings/route.ts](../../app/api/bookings/route.ts).
...
- **WhatsApp / Butler handoff + identity** - WhatChimp calls `/api/butler/lead`, Oraya persists `whatsapp_leads`, may mint a short-lived opaque `prefill_url`, `/book?h=...` hydrates safe fields only, and successful website-originated bookings best-effort back-link the originating `whatsapp_leads.linked_booking_id`. Returning-guest identity runs through `/api/butler/identify` using subscriber-id continuity first, then phone continuity for future channels, then booking reference + email/full-name proof; `/api/butler/confirmed-guest-info` returns only a narrow confirmed-guest allow-list.
- **Hosted payment execution** - `POST /api/payments/checkout` resolves the configured hosted-checkout adapter after the booking row exists, persists `payment_link_*` state on the booking, and `POST /api/payments/webhook/[provider]` is the authority for marking provider-hosted payments received.
