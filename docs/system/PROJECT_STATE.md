...
- **Phase 16B - IN PROGRESS.** Phase 16B.1 payment-link schema/domain scaffolding and 16B.2 runtime plumbing are complete. Phase 16B.3 now uses a provider-agnostic hosted checkout architecture on the Reserve path: `/book` still creates the booking first, `POST /api/payments/checkout` resolves the configured hosted-payment adapter, persists booking-side `payment_link_*` state only for providers supported by the current schema, and verified callbacks reconcile payment state through `POST /api/payments/webhook/[provider]`. Credit Libanais / MPGS is the production target provider; Stripe remains optional for dev/test only. Refunds, WhatsApp payment replies, admin payment controls, and Instant Book execution remain later 16B work. See [/docs/phases/PHASE_16B_PLAN.md](../phases/PHASE_16B_PLAN.md) and [/PHASE_16_PLAN.md](../../PHASE_16_PLAN.md).
