...
- **Phase 16A - IN PROGRESS, substantially complete.** WhatsApp AI Butler read-only foundation (`/api/butler/health|event-types|addons|availability|normalize-dates`), lead intake (`/api/butler/lead` + `whatsapp_leads` + `/admin/leads`), secure website handoff (`/api/butler/prefill` + `?h=...` on `/book`), lead -> booking identity continuity (best-effort `whatsapp_leads.linked_booking_id` writer in `/api/bookings` POST), the identity orchestrator (`/api/butler/identify` with subscriber-id primary + chat-id diagnostic + email-or-name proof + booking-view URL enrichment), the confirmed-guest info boundary (`/api/butler/confirmed-guest-info`), the website CTA prefill routing, and the inbound-message convenience (`message_text` -> bounded 8-char hex extractor) are all shipped. Outstanding 16A scope is operator-side and policy-side: WhatChimp confirmed-guest flow wiring, confirmed-guest routing refinement, ensuring the generic AI does not answer booking-sensitive confirmed-guest questions, and `POST /api/butler/flow-submit` (the write-capable booking adapter) if/when WhatsApp-side booking submission is ever re-scoped in. Location / PIN / access automation is **not** Phase 16A - it remains Phase 16D. The 8-character booking reference on `/booking/view/[token]` is a **public support code, not an access PIN**.
- **Phase 16B - IN PROGRESS, active.** Provider-agnostic hosted checkout architecture on the Reserve path is shipped: `/book` creates the booking first, `POST /api/payments/checkout` resolves the configured hosted-payment adapter, persists booking-side `payment_link_*` state, and server-side gateway verification and/or `POST /api/payments/webhook/[provider]` is the verification-authoritative reconciliation surface. The Credit Libanais provider-schema compatibility migration is staged ([sql/phase-16b4-credit-libanais-provider-compat.sql](../../sql/phase-16b4-credit-libanais-provider-compat.sql)) and the readiness contract distinguishes guest-safe from admin-safe readiness. `/admin/settings` owns guest-facing payment behavior (request-only / manual / online / hybrid mode, deposit minimum, manual rails, guest copy, provider display name) while gateway credentials remain server-only env vars. `/book` Step 3 is now the **three-step booking flow** consolidation - Villa & Dates -> Stay Setup -> Review & Guest Details - with two clear Reserve actions: **primary "Continue to secure payment"** (gold solid CTA, blocked when the provider is not truly ready) and **secondary "Reserve now, pay later"** (outline button). Add-ons and special requests do not block the stay payment - approval-based items are reviewed and charged separately after Oraya confirms. Credit Libanais / NetCommerce / CyberSource Unified Checkout is the approved production direction; the retained Stripe adapter is isolated to local / dev testing only (`PAYMENT_PROVIDER=stripe` is explicitly rejected in production). As of 2026-06-17, draft PR #64 (`agent/phase-16b-cybersource-unified-checkout-test`) has passed the Preview sandbox approved-payment path and is ready for NetCommerce-side testing: `/book` creates a booking, pay-now opens `/payments/checkout/[token]`, CyberSource Unified Checkout loads with the official `NCseal_M.png` payment seal visible, an approved sandbox card completes through `POST /api/payments/unified-checkout-complete`, and payment fields update to authorized/paid. As of 2026-06-22, temporary PR #64 Preview-only NetCommerce QA flags can also bypass review-before-payment UI gating and mark the sandbox booking `confirmed` after authoritative CyberSource approval so NetCommerce can complete their workflow; outside that explicit QA mode, `bookings.status` remains `PENDING` until Oraya admin/operations confirmation. Declined-card validation is still open because the attempted decline-style sandbox card authorized; NetCommerce/CyberSource must provide the official declined-card vector or decline trigger. PR #64 is open/draft and unmerged, production `master` is unchanged, and production payment is not enabled. Active work: NetCommerce sandbox review, declined-card vector validation, production credentials/env rollout only after approval, verified webhook execution, settlement reconciliation, refunds automation, confirmed-guest support wiring, WhatsApp payment-status replies, Instant Book execution. See [/docs/phases/PHASE_16B_PLAN.md](../phases/PHASE_16B_PLAN.md) and [/PHASE_16_PLAN.md](../../PHASE_16_PLAN.md).
- **Recovered dirty worktree note.** The old `C:\Users\David\OneDrive - Sela\Desktop\oraya-web` worktree on branch `codex/phase-16b-payment-readiness` was recovered after accidental mass deletions / generated artifacts. It now has 0 tracked deletions, 0 tracked modifications, and 0 untracked files. Generated Phase 16C export artifacts were moved outside the repo to `C:\Users\David\OneDrive - Sela\Desktop\oraya-local-backups\phase-16c-exports-from-dirty-tree`. Do not treat that old branch as pending useful Phase 16B implementation; future Phase 16B work should start from updated `master` / a clean branch after the PR #64 decision unless David explicitly instructs otherwise.
...
- **Hosted payment execution (Preview sandbox ready for external testing)** - `POST /api/payments/checkout` resolves the configured hosted-checkout adapter after the booking row exists, persists `payment_link_*` state on the booking, and server-side gateway verification and/or `POST /api/payments/webhook/[provider]` is the verification-authoritative reconciliation surface. Credit Libanais / NetCommerce / CyberSource Unified Checkout is the approved production direction; the Stripe adapter is isolated to local/dev (`PAYMENT_PROVIDER=stripe` is rejected in production). PR #64's Preview sandbox path now creates CyberSource capture contexts, loads the Unified Checkout UI, displays the official NetCommerce seal, and completes approved sandbox authorization server-side. Browser success/cancel returns remain informational. Normal server-side completion updates payment fields only and does not auto-confirm `bookings.status`; the temporary PR #64 NetCommerce QA mode is the only exception and confirms only after server-side CyberSource approval for Preview sandbox workflow validation. Declined-card sandbox validation, webhook/MLE reconciliation, settlement reconciliation, and refunds automation are NOT yet complete. `/api/payments/readiness` exposes non-secret readiness to admins.
