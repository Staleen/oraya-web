...
- **Phase 16A — IN PROGRESS as an intake + website continuation channel.** WhatsApp AI Butler read-only foundation (`/api/butler/health|event-types|addons|availability|normalize-dates`), lead intake (`/api/butler/lead` + `whatsapp_leads` + `/admin/leads`), secure website handoff (`/api/butler/prefill` + `?h=…` on `/book`), and lead → booking identity continuity (best-effort `whatsapp_leads.linked_booking_id` writer in `/api/bookings` POST) all shipped. **Product decision (2026-05-18):** WhatsApp does not create bookings — final booking submission stays on the website via `/api/bookings` POST. `POST /api/butler/flow-submit` is deferred indefinitely and is **not** required for Phase 16A closure. Outstanding 16A operational work: human-escalation routing, AI prompt tuning, Vercel env wiring. The 8-character booking reference on `/booking/view/[token]` is a **public support code, not an access PIN** — access credentials remain Phase 16D.
- **Phase 16B — PROVISIONED, no implementation. Starts only after a `bookings` row exists.** Payment + refunds architecture / schema decision / WhatsApp payment branching / admin workflow / guest workflow / refund workflow / PR-safe roadmap in [/docs/phases/PHASE_16B_PLAN.md](../phases/PHASE_16B_PLAN.md). Roadmap in [/PHASE_16_PLAN.md](../../PHASE_16_PLAN.md).
