...
- **Hosted payment execution** - `POST /api/payments/create-session` resolves the configured hosted-checkout adapter after the booking row exists, persists `payment_link_*` state on the booking, and `POST /api/payments/webhook/[provider]` is the authority for marking provider-hosted payments received.
