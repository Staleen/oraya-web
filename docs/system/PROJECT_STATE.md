...
- **Phase 16B - IN PROGRESS, active.** Provider-agnostic hosted checkout architecture on the Reserve path is shipped: `/book` creates the booking first, `POST /api/payments/checkout` resolves the configured hosted-payment adapter, persists booking-side `payment_link_*` state, and `POST /api/payments/webhook/[provider]` is the verification-authoritative reconciliation surface. The Credit Libanais provider-schema compatibility migration is staged ([sql/phase-16b4-credit-libanais-provider-compat.sql](../../sql/phase-16b4-credit-libanais-provider-compat.sql)) and the readiness contract distinguishes guest-safe from admin-safe readiness. `/admin/settings` owns guest-facing payment behavior (request-only / manual / online / hybrid mode, deposit minimum, manual rails, guest copy, provider display name) while gateway credentials remain server-only env vars. `/book` Step 3 is now the **three-step booking flow** consolidation - Villa & Dates -> Stay Setup -> Review & Guest Details - with two clear Reserve actions: **primary "Continue to secure payment"** (gold solid CTA, blocked when the provider is not truly ready) and **secondary "Reserve now, pay later"** (outline button). Add-ons and special requests do not block the stay payment - approval-based items are reviewed and charged separately after Oraya confirms. Credit Libanais / NetCommerce / MPGS is the only approved production provider; the retained Stripe adapter is isolated to local / dev testing only (`PAYMENT_PROVIDER=stripe` is explicitly rejected in production). Active work: real MPGS session creation, verified webhook execution, settlement reconciliation, refunds automation, confirmed-guest support wiring, WhatsApp payment-status replies, Instant Book execution. See [/docs/phases/PHASE_16B_PLAN.md](../phases/PHASE_16B_PLAN.md) and [/PHASE_16_PLAN.md](../../PHASE_16_PLAN.md).
...
- **Hosted payment execution (scaffolded)** - `POST /api/payments/checkout` resolves the configured hosted-checkout adapter after the booking row exists, persists `payment_link_*` state on the booking, and `POST /api/payments/webhook/[provider]` is the verification-authoritative reconciliation surface. Credit Libanais / NetCommerce / MPGS is the only approved production provider; the Stripe adapter is isolated to local/dev (`PAYMENT_PROVIDER=stripe` is rejected in production). Live MPGS session creation, verified webhook execution, settlement reconciliation, and refunds automation are NOT yet shipped - the adapter is an explicit non-faking placeholder until the bank contract lands. `/api/payments/readiness` exposes non-secret readiness to admins.
