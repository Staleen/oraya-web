...
- **Hosted payment execution (scaffolded)** - `POST /api/payments/checkout` resolves the configured hosted-checkout adapter after the booking row exists, persists `payment_link_*` state on the booking, and server-side gateway verification and/or `POST /api/payments/webhook/[provider]` is the verification-authoritative reconciliation surface. Credit Libanais / NetCommerce / CyberSource Unified Checkout is the approved production direction; the Stripe adapter is isolated to local/dev (`PAYMENT_PROVIDER=stripe` is rejected in production). CyberSource session/capture-context creation and transient-token server authorization are scaffolded for the Credit Libanais provider, but webhook/MLE reconciliation, settlement reconciliation, and refunds automation are NOT yet shipped. `/api/payments/readiness` exposes non-secret readiness to admins.
