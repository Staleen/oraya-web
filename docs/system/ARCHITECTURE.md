...
- `access_credentials` - **Phase 16D Stage A, migration committed but UNAPPLIED** ([sql/phase-16d-access-magazine.sql](../../sql/phase-16d-access-magazine.sql), human-run). One row per physical door-PIN slot in a property's magazine (Aqara A100 front door + U100 gate): property-isolated, opaque `slot_label`/`batch_label`, normal-vs-emergency class, AES-256-GCM `pin_ciphertext` (erased at destruction — CHECK-enforced) plus keyed `pin_fingerprint` (globally unique, retained forever so no PIN is ever reissued), per-lock install/deletion confirmation timestamps, set-once booking linkage with primary/backup/replacement lineage, and a `BEFORE INSERT OR UPDATE` trigger enforcing the exact lifecycle graph (`installing → available → disclosed → quarantined → deletion_pending → destroyed`; disclosed never returns to available). RLS enabled with no policies; service-role `select/insert/update` only (no delete). Server-only companions: [lib/access/pin-vault.ts](../../lib/access/pin-vault.ts) (keyring encryption + fingerprint under `ACCESS_PIN_VAULT_KEYS`/`ACCESS_PIN_FINGERPRINT_KEY`, both unset today), [lib/access/pin-generator.ts](../../lib/access/pin-generator.ts), [lib/access/pin-lifecycle.ts](../../lib/access/pin-lifecycle.ts). **No runtime consumer, UI, route, allocation, guest reveal, or Aqara integration exists yet — those are Stages B–E, each separately approval-gated.**
- `access_credential_events` - Phase 16D Stage A companion audit table (same unapplied migration): append-only lifecycle history (generated / pin_revealed_for_loading / install and deletion confirmations / disclosed / quarantined / destroyed / physical_audit_confirmed), immutable via a raising `BEFORE UPDATE OR DELETE` trigger; service-role `select/insert` only. Every PIN reveal in later stages must append a row here — an unaudited reveal path must never exist.
