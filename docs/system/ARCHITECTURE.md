...
- **No payment, smart-lock, member-linking, or booking-creation writes** (beyond the provenance enrichment above). Booking creation via WhatsApp (`POST /api/butler/flow-submit`) is still outstanding. Payment is 16B. Smart-lock is 16D. Member ↔ phone linkage is a later phase. The locked `/api/bookings*`, `/api/admin/bookings*`, `/api/calendar/*`, `/api/cron/*` surfaces remain otherwise untouched.
