...
| `/api/butler/lead` | POST | Phase 16A.2.e/f — WhatsApp/WhatChimp lead persistence into `whatsapp_leads` plus additive short-lived `/book?h=...` `prefill_url`. Lead intake only — no booking creation, no date holding, no availability check, no email. | secret-guarded |
| `/api/butler/prefill` | GET | Phase 16A.2.f — token-authenticated safe prefill projection from an existing `whatsapp_leads` row for `/book?h=...`. | token-guarded |

`secret-guarded` rows require an `X-Butler-Secret` header matching `BUTLER_WEBHOOK_SECRET`. `token-guarded` rows do not accept `X-Butler-Secret`; the signed opaque token is the auth material. Phase 16A.1 routes are read-only. Phase 16A.2.e (`/api/butler/lead`) is the **first Butler write**, but it writes only to the new `whatsapp_leads` operational table — it does not touch `bookings` or any locked surface, and the lead is not a booking. See "Butler flow" below and [DECISIONS_LOG.md](DECISIONS_LOG.md) (2026-05-12 + 2026-05-15 entries).
...
- **Write endpoints (Phase 16A.2.e, shipped).**
  - `/api/butler/lead` **POST** — WhatsApp lead persistence. Accepts a flexible JSON payload of guest details collected by the AI Butler (`oraya_full_name`, `oraya_phone`, `oraya_request_type`, `oraya_check_in_text`, `oraya_check_out_text`, `oraya_guest_count`, `oraya_villa`, `oraya_addons_interest`, `oraya_special_requests`, `labels`, optional pre-normalized dates, optional `linked_booking_id`, full `raw_payload`) and inserts one row into the new `whatsapp_leads` operational table via [lib/butler/leads.ts](../../lib/butler/leads.ts) `normalizeLeadInput`. Returns `{ ok: true, lead_id, message, prefill_url }`; `prefill_url` is `{SITE_URL}/book?h=<token>` where the token is a 2-hour HMAC-SHA256 opaque prefill token signed with `BUTLER_PREFILL_SECRET`. **The lead is NOT a booking** — no row is created in `bookings`, no availability is held, no email is sent, no payment is triggered. Operators triage from `/admin/leads`. Raw Supabase / driver errors are logged server-side and replaced with a safe `{ ok: false, error: "server_error" }` 500.
  - `/api/butler/prefill` **GET** — public token-authenticated prefill projection for `/book?h=...`. Verifies the token, reads the existing `whatsapp_leads` row by `lead_id`, and returns only `villa`, `check_in`, `check_out`, `sleeping_guests`, `full_name`, and `source`. It never returns phone, `raw_payload`, notes, labels, follow-up status, or linked booking IDs. Responses use `Cache-Control: no-store`.
...
- **Website handoff.** `/book?h=<token>` fetches `/api/butler/prefill`, hydrates only empty/current-safe booking fields, preserves existing `?villa=` behavior, lets token villa prefill win when both query params exist, and strips `h` from the URL after hydration succeeds or fails.
- **No payment, smart-lock, member-linking, or booking-creation writes.** Booking creation via WhatsApp (`POST /api/butler/flow-submit`) is still outstanding. Payment is 16B. Smart-lock is 16D. Member ↔ phone linkage is a later phase. The locked `/api/bookings*`, `/api/admin/bookings*`, `/api/calendar/*`, `/api/cron/*` surfaces remain untouched.
...
- **Server-only** (Node runtime only, never in client components or `NEXT_PUBLIC_*`): `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `BOOKING_ACTION_SECRET`, `CRON_SECRET`, `ADMIN_SECRET`, `BUTLER_WEBHOOK_SECRET`, `BUTLER_PREFILL_SECRET`.
