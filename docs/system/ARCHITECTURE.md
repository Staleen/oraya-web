...
| `/api/payments/webhook/[provider]` | POST | Verified hosted-payment callback reconciliation. For `credit_libanais`, the fail-closed handler decrypts payment/Unified Checkout compact JWE, verifies the separate timestamped `v-c-signature` over the decrypted payload with a five-minute tolerance, durably claims a stable replay key in `payment_provider_events`, then reconciles the matching monotonic `payment_attempts` row. Missing config returns 503; decryption/signature failures return 401; no browser redirect can create payment truth | payment |
...
3. **Step 2 — Stay Setup.** Bedrooms, guests, add-ons, special requests; live estimated total via [lib/pricing/](../../lib/pricing/) helpers. The current hosted-checkout eligibility path is intentionally narrow: any selected add-on or typed special request keeps the booking request but routes the primary pay-now choice to the pending-payment follow-up instead of opening hosted checkout. Approval-based items are reviewed and charged separately after Oraya confirms.
4. **Step 3 — Review & Guest Details.** Review summary, guest-details form (Reserve path), and payment decision. Step 3 presents two explicit Reserve actions:
   - **Primary:** **"Continue to secure payment"** — solid gold CTA. When clicked, `submitIntent = "pay_now"`. The flow first calls `POST /api/bookings` (the locked booking-creation authority). It calls `POST /api/payments/checkout` only when hosted checkout and the stay are eligible; otherwise it preserves the request and routes to `/booking/view/[token]?payment=pending` for Oraya follow-up (no fake checkout and no technical readiness text).
...
- **Confirmed-guest info boundary (shipped).** `POST /api/butler/confirmed-guest-info` is the secret-guarded surface that returns the Phase 16A allow-list of fields a confirmed, identity-established guest is permitted to receive: public booking reference, villa name, check-in/check-out dates, signed `/booking/view/[token]` URL, high-level `checkin_guidance` (operator-managed via the `butler_checkin_guidance` settings key — placeholder when unset), an explicit `location_access_note` that exact location, gate codes, and smart-lock access are not yet provided and remain a Phase 16D approval-gated concern, and — Phase 16C Stage 4B — `arrival_guide_url`: the personalized private Arrival Guide link (`https://stayoraya.com/arrival/<signed-view-token>`, existing signed `view` token with checkout-day expiry minted by [lib/arrival-guide-link.ts](../../lib/arrival-guide-link.ts)), returned only when identity is established, the booking is confirmed, `check_out` is usable, and the villa resolves to a supported Butler villa; `null` on every refusal branch. The guide contains stay details, villa guidance, maps, and local recommendations — no access codes. Reuses `orchestrateButlerIdentity` for the identity decision and refuses pending / cancelled / unverified bookings without surfacing any sensitive structured field. Never returns: PIN, exact GPS, payment links, admin notes, internal IDs.
...
- `access_credentials` - **Phase 16D Stage A, migration committed but UNAPPLIED** ([sql/phase-16d-access-magazine.sql](../../sql/phase-16d-access-magazine.sql), human-run). One row per physical door-PIN slot in a property's magazine (Aqara A100 front door + U100 gate): property-isolated, opaque `slot_label`/`batch_label`, normal-vs-emergency class, AES-256-GCM `pin_ciphertext` (erased at destruction — CHECK-enforced) plus versioned keyed `pin_fingerprint` + `pin_fingerprint_key_id` (globally unique, retained forever so no PIN is ever reissued even across fingerprint-key rotation), per-lock install/deletion confirmations as write-once **timestamp + actor pairs** (`available` requires both complete install pairs; `destroyed` requires complete deletion pairs for every installed lock), comprehensive state/fact-consistency CHECKs (disclosure all-or-none and absent before disclosure; quarantine pair exactly from quarantine onward; deletion facts only from quarantine onward; allow-listed `quarantine_reason` codes), set-once booking linkage with primary/backup/replacement lineage **verified in-database against the predecessor row** (same property + booking, consumed predecessor, valid relationship, no self-reference), and a `BEFORE INSERT OR UPDATE` trigger enforcing the exact lifecycle graph (`installing → available → disclosed → quarantined → deletion_pending → destroyed`; disclosed never returns to available). RLS enabled with no policies; service-role `select/insert/update` only (no delete). Server-only companions: [lib/access/pin-vault.ts](../../lib/access/pin-vault.ts) (encryption keyring `ACCESS_PIN_VAULT_KEYS` + versioned fingerprint keyring `ACCESS_PIN_FINGERPRINT_KEYS`, both unset today; runtime browser-context guard), [lib/access/pin-generator.ts](../../lib/access/pin-generator.ts), [lib/access/pin-lifecycle.ts](../../lib/access/pin-lifecycle.ts). **No runtime consumer, UI, route, allocation, guest reveal, or Aqara integration exists yet — those are Stages B–E, each separately approval-gated.**
- `access_credential_events` - Phase 16D Stage A companion audit table (same unapplied migration): append-only lifecycle history (generated / pin_revealed_for_loading / per-lock install and deletion confirmations / became_available / disclosed / quarantined / deletion_started / destroyed / physical_audit_confirmed), immutable via a raising `BEFORE UPDATE OR DELETE` trigger; service-role `select/insert` only; the only variable field is an allow-listed `reason_code` (no free text, so no audit surface can accidentally persist a PIN). **Every credential-row mutation writes its event transactionally via an AFTER trigger — direct service-role updates cannot bypass the ledger**; the one explicitly app-inserted event class is `pin_revealed_for_loading` (reveals are reads, not mutations), so an unaudited reveal path must never exist.
