...
| `/api/payments/create-session` | POST | Additive production-facing alias for the hosted checkout session creator; returns `provider_not_configured` while the selected provider is placeholder-only | payment |
...
6. After booking creation succeeds, `POST /api/payments/create-session` resolves the configured hosted-checkout adapter through the existing checkout handler, creates a provider session when the adapter is fully implemented, persists `payment_link_*` state on the booking row, and returns the hosted checkout URL.
