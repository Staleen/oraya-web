...
- **Auth.** Every secret-guarded `/api/butler/*` route is guarded by [lib/butler/auth.ts](../../lib/butler/auth.ts) `requireButlerAuth`, which validates an `X-Butler-Secret` header against `BUTLER_WEBHOOK_SECRET` using a constant-time compare. 503 if the env is unset; 401 if the header is missing or wrong. HMAC + timestamp remains a future hardening step once WhatChimp's outbound-signing posture is confirmed. `/api/butler/prefill` is the public exception: it is token-authenticated with `BUTLER_PREFILL_SECRET`, not `X-Butler-Secret`.
